...
import gameParams, { listAwards, innerGamified } from "./params";
...
const store = new Vuex.Store({
  ...
  getters: {
    gamified: (state) => innerGamified(state.dayNumber),
    status: (state) => state.socket.isConnected,
    getProb: (state) => () => {
      const { dayLength, bonusProbabilityCoef } = gameParams;

      return (
        (state.secSpentOnTrade / dayLength) *
        bonusProbabilityCoef *
        100
      ).toFixed(2);
    },
    dataInLoading: (state) => () => {
    ...
    pandle: (state) => (name) => {
      let avBuyPrice = 0,
        avSellPrice = 0;
      const sells = _.filter(
        state.transactions,
        (i) => i.innerName === name && i.inner_action == "sell"
      );
      const buys = _.filter(
        state.transactions,
        (i) => i.innerName === name && i.inner_action == "buy"
      );
      const totSells = _.sumBy(sells, function(o) {
        return o.price * o.quantity;
      });
      const totSellsQ = _.sumBy(sells, function(o) {
        return o.quantity;
      });
      if (sells.length > 0) {
      ...
      const totBuys = _.sumBy(buys, function(o) {
        return o.price * o.quantity;
      });
      const totBuysQ = _.sumBy(buys, function(o) {
        return o.quantity;
      });
    ...
    getCurrentTransactionNum: (state) => () => {
      return state.numTransactions;
    },
    getCurrentTimeInTrade: (state) => () => {
      return state.secSpentOnTrade;
    },
    getCashBalance: (state) => () => {
      return state.cashBalance;
    },
    portfoglioValue: (state) => () => {
      return _.sumBy(state.stocks, (i) => {
  ...
  mutations: {
    ...
    INCREASE_TOTAL_TASKS_COUNTER: (state) => {
      state.tasksSubmitted++;
    },
    INCREASE_CORRECT_TASKS_COUNTER: (state) => {
      state.correctTasksSubmitted++;
    },
    SET_NEW_TASK: (state, body) => {
  ...
  actions: {
    setNumAward({ commit, getters, state, dispatch }, obj) {
      if (getters.gamified) {
        commit("SET_NUM_AWARD", obj);
        dispatch("sendEventToServer", {
          name: "awardForTransaction",
          award_type: obj,
          numTransactions: state.numTransactions,
        });
      }
    },
    setTimeAward({ commit, state, getters, dispatch }, obj) {
      if (getters.gamified) {
        commit("SET_TIME_AWARD", obj);
        dispatch("sendEventToServer", {
          name: "awardForTime",
          secSpentOnTrade: state.secSpentOnTrade,
          award_type: obj,
        });
      }
    },
    ...
    makeTransaction(
      ...
      if (initial || state.cashBalance + finalAmount > 0) {
        ...
        const allPrices = state.stocks.map(({ innerName, price }) => ({
          innerName,
          price,
        }));
    ...
    async dayEnds({ commit, state, getters, dispatch }) {
    ...
    async nextDay({ commit, state, dispatch }, { startSession = false } = {}) {
      ...
      const specificDayParams = _.find(
        day_params,
        _.matches({ round: next_one })
      );
      ...
      if (specificDayParams === undefined) {
        ...
        dispatch("updStocks");
        await dispatch("sendEventToServer", {
          name: "dayStarted",
    ...
    clearHoldings({ commit, state, getters, dispatch }) {
      ...
      dispatch("sendEventToServer", {
        name: "clearingStockBeforeDayEnds",
        finalStockAmount,
        stocksOwned: stocks,
        balance: state.cashBalance,
      });
    ...
    giveBonusForTime({ commit, dispatch, state }) {
      ...
      if (r <= prob) {
        ...
        dispatch("sendEventToServer", {
          name: "giveBonus",
          chosenStockName: chosenStock.innerName,
    ...
    async sendEventToServer(context, message) {
      try {
        ...
        console.debug("TRYING TO SEND SOMETHING ON SEVER WHAT?", message);
      }
    },
  },
});
